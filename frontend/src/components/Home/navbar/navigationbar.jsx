...
const NavigationBar = () => {
    ...
    // console.log( isOpen );
    return (
        <div className='fixed w-full border-b-2 border-gray bg-gray-50 z-10'>
            <nav className="container mx-auto px-4 py-2 flex justify-between items-center ">
                ...
                <ul className="hidden lg:mx-auto lg:flex lg:items-center lg:w-auto lg:space-x-6 gap-3">
                    { navLinks.map( ( link, index ) => {
                        // Check if the link is "Quiz" and the user is not logged in, then skip rendering
                ...
                {/* <div id='navbar-profile' className='hidden lg:flex items-center sm:hidden gap-3'>
                    ...
                    { HideDropList &&
                        ...
                        } className={ `bg-gray-100 transition-opacity ${ HideDropList ? 'transition ease-out duration-100 transform'
                        ...
                        >

                            {
                                user.role === 'patient' ? (
                                    <div className='rounded-lg hover:bg-gray-200 hover:rounded-lg py-1'>
                                        <Link to="/Dashboard" className="flex items-center text-center gap-4 text-gray-700  px-4 py-2 text-md text-lg">
                                    ...
                                    (
                                        user.role === 'doctor' ? (
                                            <div className='rounded-lg hover:bg-gray-200 hover:rounded-lg py-1'>
                                                <Link to="/Dashboard/doctor" className="flex items-center text-center gap-4 text-gray-700  px-4 py-2 text-md text-lg">
                                                    <img src={ dashboard } className='w-7' alt="" />
                                                    Dashboard
                                                </Link>
                                            </div>
                                            ...
                                            <div className='rounded-lg hover:bg-gray-200 hover:rounded-lg py-1'>
                                                <Link to="/Dashboard/admin" className="flex items-center text-center gap-4 text-gray-700  px-4 py-2 text-md text-lg">
                                                    <img src={ dashboard } className='w-7' alt="" />
                                                    Dashboard
                                                </Link>
                                            </div>
                            ...
                            <div className='rounded-lg hover:bg-gray-200 hover:rounded-lg py-1'>
                                <Link to="/create-pin" className="flex items-center text-center gap-4 text-gray-700  px-4 py-2 text-md">
                ...
                {/* )
                        ...
                        ( */}
                <div id='navbar-profile' className='hidden lg:flex items-center sm:hidden gap-3'>
                    <Link to="/Auth/register" className={ `hidden lg:inline-block lg:ml-auto lg:mr-3 py-2 px-6 bg-gray-100 hover:bg-gray-100 text-gray-900 font-bold rounded-xl transition duration-200 ${ activeLink === '/Auth/register' ? 'font-bold text-blue-500' : '' }` }>
                        Sign In
                    </Link>
                    <Link to="/Auth/login" className={ `hidden lg:inline-block py-2 px-6 bg-blue-500 hover:bg-blue-600 text-lg text-white font-bold rounded-xl transition duration-200 ${ activeLink === '/Auth/login' ? 'font-bold text-blue-500' : '' }` }>
                        Sign Up
                    </Link>
                </div>
                {/* )
                    ...
                    <div class="lg:hidden navbar-menu relative z-50 ">
                        ...
                        <nav class="fixed top-0 left-0 bottom-0 flex flex-col w-5/6 max-w-sm py-6 px-6 bg-white border-r overflow-y-auto">
                            <div class="flex items-center mb-8">
                                <Link to={ '/' } class="mr-auto text-3xl font-bold leading-none" href="#">
                                    <span>GameGuard.</span>
                                </Link>
                                        ...
                                        user ?
                                            (
                                                <div id='navbar-profile' className='flex items-center p-4'>
                                                    <Link to={ `/Dashboard/Profile/${ user.userId }` } className="block">
                                            ...
                                            (
                                                <>
                                                    <Link to="/Auth/register" class="block px-4 py-3 mb-3  text-lg text-center font-semibold  bg-gray-50 hover:bg-gray-100 rounded-xl" >Sign </Link>
                                                    <Link to="/Auth/login" class="block px-4 py-3 mb-2  text-lg text-center text-white font-semibold bg-blue-600 hover:bg-blue-700  rounded-xl" >Sign In</Link>
                                                </>
                                            )
                                    }
                                </div>
